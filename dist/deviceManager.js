...
import { buildDeviceListPacket } from "./protocol.js";
...
export function getDeviceSummaries() {
...
export function getAllDeviceSessions() {
    return Array.from(devices.values());
}
async function processB64FrameAsync(dev, b64) {
    try {
        ...
        console.warn(`[device] Failed to process frame for ${dev.deviceId}: ${e.message}`);
    }
}
export async function ensureDeviceAsync(id, cfg, attach = false) {
    const root = getRoot();
    if (!root)
    ...
    if (device) {
        if (attach || deviceConfigsEqual(device.cfg, cfg)) {
            try {
                ...
                try {
                    const { data: b64 } = await device.cdp.send('Page.captureScreenshot', { format: 'png' });
                    await processB64FrameAsync(device, b64);
                }
                catch (e) {
                    console.warn(`[device] captureScreenshot failed for ${id}: ${e.message}`);
                }
                if (device.url)
                    broadcaster.sendCurrentURL(id, device.url);
                ...
                console.warn(`[device] CDP session broken for ${id}, recreating: ${e.message}`);
...
export async function killDeviceAsync(id) {
    const dev = devices.get(id);
    if (!dev)
        return;
