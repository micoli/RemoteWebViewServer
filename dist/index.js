...
wss.on("connection", async (ws, req) => {
    ...
    if (isBrowser)
        broadcaster.registerBrowserClient(ws);
    broadcaster.addClient(id, ws, isBrowser);
    // Buffer messages arriving before the device session is ready
    const earlyMessages = [];
    const earlyHandler = (msg, isBinary) => earlyMessages.push({ msg, isBinary });
    ...
    try {
        ...
        console.error(`[server] setup failed for ${id}, closing connection:`, e.message);
    ...
    if (isBrowser)
        broadcaster.sendToBrowserClient(ws, buildDeviceListPacket(getDeviceSummaries()));
    ws.off("message", earlyHandler);
    const dispatch = (msg, isBinary) => {
        if (!isBinary)
            return;
        ...
        switch (buf.readUInt8(0)) {
            case MsgType.Touch: {
                ...
                if (clientCount > 1)
                    console.warn(`[server] ${clientCount} WS connections for id=${id} — possible duplicate`);
                inputRouter.handleTouchPacketAsync(dev, buf).catch(e => console.warn(`Failed to handle touch packet: ${e.message}`));
                break;
            }
            ...
            case MsgType.OpenURL:
                inputRouter.handleOpenURLPacketAsync(dev, buf).catch(e => console.warn(`Failed to handle OpenURL packet: ${e.message}`));
                break;
            case MsgType.KillDevice: {
                const targetId = parseKillDevicePacket(buf);
                if (targetId)
                    killDeviceAsync(targetId).catch(e => console.warn(`Failed to kill device ${targetId}: ${e.message}`));
                break;
            }
        }
    };
